...
>La pregunta **bonus** no es obligatoria, pero en caso de ser correcta recibe una bonificación para la nota.

Realize un modelo de regresión múltiple para determinar si los niveles socioeconómicos (`NSE`) predicen de forma estadisticamente significativa la variable de respuesta `matrimonio`. Muestre los resultados del modelo usando el comando `summary()`. Transforme los datos de manera que el modelo de regresión tenga las siguientes caracteristicas:

  1. Las pruebas de hipótesis de cada media estimada este evaluando si esta es distinta del promedio global.
  2. Le permita estimar las distancias de las medias respecto al promedio global de forma simultánea.  
